...
Matriz *CriarMatriz(int i, int j)   //Funcao que cria a matriz
{
    Matriz *A;
...
int IniciaCabecas(Matriz *A)    //Funcao que cria a cabeca principal
{
    int i=0;
...
int InsereCabecaColuna(Matriz *A)   //Funcao que cria as cabecas da coluna
{
    Celula *cabeca;
    cabeca = (Celula*)malloc(sizeof(Celula));
...
int InsereCabecaLinha(Matriz *A)    //Funcao que cria as cabecas da linha
{
    Celula *cabeca;
    cabeca = (Celula*)malloc(sizeof(Celula));
...
void Insere(int i, int j, double valor, Matriz *A)  //Funcao que insere um valor numa determinada posicao || i = Linha || j = Coluna
{
    int m;
...
void ImprimeMatriz(Matriz *A)   //Funcao que imprime a matriz
{
    int m, n;
    Celula *pCelula;
    if (!A || !A->i || !A->j)return;
...
int ApagaMatriz(Matriz *A)  //Funcao que apaga a matriz
{
    int m = 1;
    int n = 1;
    Celula *pCelula, *aux;
...
Matriz *SomaMatriz(Matriz *A, Matriz *B)    //Funcao que soma a matriz
{
    int m, n;
    float soma;
    Matriz *matC;
...
Matriz *MultiplicaMatriz(Matriz *A, Matriz*B)   //Fucao que multiplica a matriz
{
    int m=0, n=0, k=0;
    float total;
    Matriz *matC;
...
double ObtemElementoPeloIndice(Matriz *A, int i, int j) //Funcao que retorna o valor de um determinada posicao
{
    Celula *pCelula;
    int m = 0;
    pCelula = A->inicio->direita;
...
Matriz *LeMatriz(char *nome)   //Funcao que le um arquivo e retorna a matriz
{
    Matriz *A;
